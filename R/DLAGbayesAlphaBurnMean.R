...
DLAGbayesAlphaBothBurn = function(y, x, c=NULL,
  ...
  logTauVec = c(seq(-20, -10, by=1),
                ...
                seq(-0.00000000001, -0.000000000001, by=0.000000000001),
                seq(-0.000000000001, -0.0000000000001, by=0.0000000000001),
                seq(-0.0000000000001, -0.00000000000001, by=0.00000000000001),
                seq(-0.00000000000001, -0.000000000000001, by=0.000000000000001))
  tauVec = exp(logTauVec)

  tauStore = matrix(0.5, nChains, p)
  tauIntStore = matrix(0.5, nChains, choose(p,2))

  tauStoreTemp = array(0.5, dim=c(nChains, nBurn - floor(nBurn/2), p))
  tauIntStoreTemp = array(0.5, dim=c(nChains, nBurn - floor(nBurn/2), choose(p,2)))
  ...
  for (ni in 2 : nScans) {
    for (nc in 1 : nChains) {
      ...
      for (j in 1 : p) {
        ...
        if (ni < (nBurn/2)) {
          tau = rbeta(1, 3 + sum(gammaPost[nc,ni-1,] == 1),
                      p + sum(gammaPost[nc,ni-1,] == 0))
        } else if (ni < nBurn) {
          ...
          tauStoreTemp[nc,ni - floor(nBurn/2),j] = tau
        } else if (ni == nBurn) {
          tauStore[nc,j] = mean(tauStoreTemp[nc,,j])
          tau = tauStore[nc,j]
        } else {
          tau = tauStore[nc,j]
        }
        ...
        p1 = log(tau) + dmvn0Ander(mean=mu, sigma=PriorSigma) -
          dmvn0Ander(mean=muBeta, sigma=muVar)
      ...
      for (j in 1 : choose(p,2)) {
        ...
        if (ni < (nBurn/2)) {
          tau = rbeta(1, 3 + sum(gammaIntPost[nc,ni-1,] == 1),
                      p + sum(gammaIntPost[nc,ni-1,] == 0))
        } else if (ni < nBurn) {
          logRATIOvec = rep(NA, 100)
          for (rr in 1 : length(logRATIOvec)) {
          ...
          tauIntStoreTemp[nc,ni - floor(nBurn/2),j] = tau
        } else if (ni == nBurn) {
          tauIntStore[nc,j] = mean(tauIntStoreTemp[nc,,j])
          tau = tauIntStore[nc,j]
        } else {
          tau = tauIntStore[nc,j]
        }
